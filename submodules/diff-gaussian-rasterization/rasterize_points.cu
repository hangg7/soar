...
RasterizeGaussiansCUDA(
  ...
  if (P != 0) {
    ...
    rendered = CudaRasterizer::Rasterizer::forward(
        ...
        patchbbox.contiguous().data<float>(), campos.contiguous().data<float>(),
        tan_fovx, tan_fovy, prefiltered, render_front, sort_descending,
        config.contiguous().data<float>(), out_color.contiguous().data<float>(),
        out_normal.contiguous().data<float>(),
        out_depth.contiguous().data<float>(),
        out_opac.contiguous().data<float>(), radii.contiguous().data<int>(),
        debug);
  }
  return std::make_tuple(rendered, out_color, out_normal, out_depth, out_opac,
...
RasterizeGaussiansBackwardCUDA(
  ...
  if (P != 0) {
    CudaRasterizer::Rasterizer::backward(
        ...
        viewmatrix.contiguous().data<float>(),
        projmatrix.contiguous().data<float>(),
        campos.contiguous().data<float>(), prcppoint.contiguous().data<float>(),
        patchbbox.contiguous().data<float>(), tan_fovx, tan_fovy,
        ...
        dL_dcampos.contiguous().data<float>(), debug,
