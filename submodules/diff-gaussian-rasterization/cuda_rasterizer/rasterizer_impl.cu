...
__global__ void checkFrustum(int P, const float *orig_points,
  ...
  // float3 p_view;
  // present[idx] = in_frustum(idx, orig_points, viewmatrix, projmatrix, false,
  // p_view);
}
...
int CudaRasterizer::Rasterizer::forward(
    ...
    const float *projmatrix, const float *prcppoint, const float *patchbbox,
    const float *cam_pos, const float tan_fovx, float tan_fovy,
    const bool prefiltered, const bool render_front, const bool sort_descending,
    float *config, float *out_color, float *out_normal, float *out_depth,
    float *out_opac, int *radii, bool debug) {
  ...
  CHECK_CUDA(FORWARD::preprocess(
                 ...
                 prcppoint, patchbbox, (glm::vec3 *)cam_pos, width, height,
                 focal_x, focal_y, tan_fovx, tan_fovy, radii, geomState.means2D,
                 geomState.depths, geomState.cov3D, geomState.rgb,
                 geomState.normal, geomState.conic_opacity, geomState.Jinv,
                 geomState.viewCos, geomState.pid, geomState.pview, tile_grid,
                 geomState.tiles_touched, prefiltered, render_front, config),
             debug)
  ...
  CHECK_CUDA(FORWARD::render(
                 ...
                 width, height, prcppoint, patchbbox, focal_x, focal_y,
                 geomState.means2D, feature_ptr, geomState.normal,
                 geomState.depths, geomState.conic_opacity, geomState.Jinv,
                 geomState.pid, geomState.pview, imgState.accum_alpha,
                 imgState.accum_depth, imgState.accum_color, imgState.accum_var,
                 imgState.n_contrib, imgState.accum_alpha_cut,
                 imgState.n_contrib_cut, background, out_color, out_normal,
                 out_depth, out_opac, config),
             debug)
  return num_rendered;
}
...
void CudaRasterizer::Rasterizer::backward(
    ...
    const float *cov3D_precomp, const float *viewmatrix,
    const float *projmatrix, const float *campos, const float *prcppoint,
    const float *patchbbox, const float tan_fovx, float tan_fovy,
    const int *radii, char *geom_buffer, char *binning_buffer, char *img_buffer,
    const float *dL_dpixcolor, const float *dL_dpixnormal,
    const float *dL_dpixdepth, const float *dL_dpixopac, float *dL_dmean2D,
    float *dL_dconic, float *dL_dopacity, float *dL_dcolor, float *dL_dnormal,
    float *dL_ddepth, float *dL_dmean3D, float *dL_dcov3D, float *dL_dsh,
    float *dL_dscale, float *dL_drot, float *dL_dviewmat, float *dL_dprojmat,
    float *dL_dcampos, bool debug, float *config) {
  ...
  CHECK_CUDA(BACKWARD::render(
                 tile_grid, block, imgState.ranges, binningState.point_list,
                 width, height, patchbbox, background, geomState.means2D,
                 geomState.conic_opacity, color_ptr, geomState.normal,
                 geomState.depths, geomState.Jinv, geomState.viewCos,
                 imgState.accum_alpha, imgState.accum_depth,
                 imgState.accum_color, imgState.accum_var, imgState.n_contrib,
                 imgState.accum_alpha_cut, imgState.n_contrib_cut, dL_dpixcolor,
                 dL_dpixnormal, dL_dpixdepth, dL_dpixopac, (float3 *)dL_dmean2D,
                 (float4 *)dL_dconic, dL_dopacity, dL_dcolor, dL_dnormal,
                 dL_ddepth, config),
             debug)
  ...
  CHECK_CUDA(BACKWARD::preprocess(
                 ...
                 (glm::vec3 *)dL_dmean3D, dL_dcolor, dL_dnormal, dL_ddepth,
                 dL_dcov3D, dL_dsh, (glm::vec3 *)dL_dscale,
                 (glm::vec4 *)dL_drot, dL_dviewmat, dL_dprojmat, dL_dcampos,
                 config),
             debug)
}
