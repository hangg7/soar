	...
	float* config)
{
	...
	// if (render_front){
	// 	printf("front p_proj: %.5f, %.5f, %.5f; p_hom: %.5f, %.5f, %.5f, %.5f\n", p_proj.x, p_proj.y, p_proj.z, p_hom.x, p_hom.y, p_hom.z, p_hom.w);
	// } else {
	// 	printf("back p_proj: %.5f, %.5f, %.5f; p_hom: %.5f, %.5f, %.5f, %.5f\n", p_proj.x, p_proj.y, p_proj.z, p_hom.x, p_hom.y, p_hom.z, p_hom.w);
	// }
	...
	if (surface) {
		...
		// printf("here\n");
		bool front = front_facing(n_view, p_view, &viewCos[idx], prefiltered);
		if (render_front && !front) return; // cull backfacing points
		// if (!render_front && front) return; // cull frontfacing points
	...
	// pview[idx] = p_view;
	// if (!render_front) {
	//     // This is a hack, and the depth will be wrong in this way.
	//     printf("back p_view: %.5f\n", p_view.z);
	//     depths[idx] = -p_view.z;
	// }
	// else {
	//     depths[idx] = p_view.z;
	...
	float* config)
{
	...
	// float distortion = {0}, dist1 = {0}, dist2 = {0};
