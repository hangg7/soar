...
def estimate_translation_np(
    S, joints_2d, joints_conf, focal_length=5000, img_size=(224.0, 224.0)
):
...
def estimate_translation(
    S, joints_2d, focal_length=5000.0, img_size=224.0, use_all_kps=False
):
    """Find camera translation that brings 3D joints S closest to 2D the corresponding joints_2d.
    Input:
...
def compute_twist_rotation(rotation_matrix, twist_axis):
    ...
    projection = (
        torch.einsum("bi,bi->b", twist_axis, quaternion[:, 1:]).unsqueeze(-1)
        * twist_axis
    )
