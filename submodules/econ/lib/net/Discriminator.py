...
class DiscriminatorHead(nn.Module):
    def __init__(self, in_channel, disc_stddev=False):
        ...
        self.final_linear = nn.Sequential(
            nn.Flatten(),
            EqualLinear(
                in_channel=in_channel * 4 * 4, out_channel=in_channel, activate=True
            ),
...
class StyleDiscriminator(nn.Module):
    def __init__(self, in_channel, in_res, ch_mul=64, ch_max=512, **kwargs):
        ...
        self.conv_in = ConvLayer2d(
            in_channel=in_channel, out_channel=ch_mul, kernel_size=3
        )
        ...
        for i in range(log_size_in, log_size_out, -1):
            ...
            self.layers.append(
                ConvResBlock2d(
                    in_channel=in_channels, out_channel=out_channels, downsample=True
                )
            )
...
class EqualLinear(nn.Module):
    ...
    def __init__(
        self, in_channel, out_channel, bias=True, bias_init=0, lr_mul=1, activate=False
    ):
        super().__init__()
...
class ConvLayer2d(nn.Sequential):
    def __init__(
        ...
        ), "Cannot upsample and downsample simultaneously"
        layers = []
...
class ConvResBlock2d(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, upsample=False, downsample=False):
        ...
        self.conv2 = ConvLayer2d(
            mid_ch, out_channel, downsample=downsample, kernel_size=3
        )
