...
class IFGeoNet(nn.Module):
    ...
    def forward(self, batch):
        ...
        p = orthogonal(batch["samples_geo"].permute(0, 2, 1), batch["calib"]).permute(
            0, 2, 1
        )  # [2, 60000, 3]
        ...
        feature_0_partial = F.grid_sample(
            x, p, padding_mode="border", align_corners=True
        )
        ...
        # Feature fusion
        net = self.actvn(
            self.conv_0_fusion(torch.concat([net_partial, net_smpl], dim=1))
        )
        net = self.actvn(self.conv_0_1_fusion(net))
        net = self.conv0_1_bn_fusion(net)
        feature_1_fused = F.grid_sample(
