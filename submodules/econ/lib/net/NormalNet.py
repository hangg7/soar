...
class NormalNet(BasePIFuNet):
    ...
    def forward(self, in_tensor):
        ...
        mask = (
            (in_tensor["image"].abs().sum(dim=1, keepdim=True) != 0.0).detach().float()
        )
    ...
    def get_norm_error(self, prd_F, prd_B, tgt):
        ...
        if "l1" in self.F_losses:
            ...
            total_loss["netF"] += (
                self.F_losses_ratio[self.F_losses.index("l1")] * l1_F_loss
            )
            total_loss["l1_F"] = (
                self.F_losses_ratio[self.F_losses.index("l1")] * l1_F_loss
            )
        if "l1" in self.B_losses:
            ...
            total_loss["netB"] += (
                self.B_losses_ratio[self.B_losses.index("l1")] * l1_B_loss
            )
            total_loss["l1_B"] = (
                self.B_losses_ratio[self.B_losses.index("l1")] * l1_B_loss
            )
        ...
        if "vgg" in self.F_losses:
            ...
            total_loss["netF"] += (
                self.F_losses_ratio[self.F_losses.index("vgg")] * vgg_F_loss
            )
            total_loss["vgg_F"] = (
                self.F_losses_ratio[self.F_losses.index("vgg")] * vgg_F_loss
            )
        if "vgg" in self.B_losses:
            ...
            total_loss["netB"] += (
                self.B_losses_ratio[self.B_losses.index("vgg")] * vgg_B_loss
            )
            total_loss["vgg_B"] = (
                self.B_losses_ratio[self.B_losses.index("vgg")] * vgg_B_loss
            )
        ...
        if "mrf" in self.F_losses:
            mrf_F_loss = self.mrf_loss(
                F.interpolate(
                    prd_F, scale_factor=scale_factor, mode="bicubic", align_corners=True
                ),
                F.interpolate(
                    tgt_F, scale_factor=scale_factor, mode="bicubic", align_corners=True
                ),
            )
            total_loss["netF"] += (
                self.F_losses_ratio[self.F_losses.index("mrf")] * mrf_F_loss
            )
            total_loss["mrf_F"] = (
                self.F_losses_ratio[self.F_losses.index("mrf")] * mrf_F_loss
            )
        if "mrf" in self.B_losses:
            mrf_B_loss = self.mrf_loss(
                F.interpolate(
                    prd_B, scale_factor=scale_factor, mode="bicubic", align_corners=True
                ),
                F.interpolate(
                    tgt_B, scale_factor=scale_factor, mode="bicubic", align_corners=True
                ),
            )
            total_loss["netB"] += (
                self.B_losses_ratio[self.B_losses.index("mrf")] * mrf_B_loss
            )
            total_loss["mrf_B"] = (
                self.B_losses_ratio[self.B_losses.index("mrf")] * mrf_B_loss
            )
        ...
        if "gan" in self.ALL_losses:
            ...
            total_loss["D_fake"] = (
                loss_D_fake * self.B_losses_ratio[self.B_losses.index("gan")]
            )
            total_loss["D_real"] = (
                loss_D_real * self.B_losses_ratio[self.B_losses.index("gan")]
            )

            total_loss["netB"] += (
                loss_G_fake * self.B_losses_ratio[self.B_losses.index("gan")]
            )
            total_loss["G_fake"] = (
                loss_G_fake * self.B_losses_ratio[self.B_losses.index("gan")]
            )
            ...
            if "gan_feat" in self.ALL_losses:
                ...
                for i in range(2):
                    for j in range(len(pred_fake[i]) - 1):
                        loss_G_GAN_Feat += self.l1_loss(
                            pred_fake[i][j], pred_real[i][j].detach()
                        )
                total_loss["netB"] += (
                    loss_G_GAN_Feat
                    * self.B_losses_ratio[self.B_losses.index("gan_feat")]
                )
                total_loss["G_GAN_Feat"] = (
                    loss_G_GAN_Feat
                    * self.B_losses_ratio[self.B_losses.index("gan_feat")]
                )
