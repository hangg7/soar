...
class Voxelization(nn.Module):
    ...
    def update_param(self, voxel_faces):
        ...
        smpl_vertex_code_batch = torch.tile(
            self.smpl_vertex_code, (self.batch_size, 1, 1)
        )
        smpl_face_code_batch = torch.tile(self.smpl_face_code, (self.batch_size, 1, 1))
        smpl_face_indices_batch = torch.tile(
            self.smpl_face_indices, (self.batch_size, 1, 1)
        )
        ...
        smpl_tetraderon_indices_batch = self.smpl_tetraderon_indices.contiguous().to(
            self.device
        )
        ...
        self.register_buffer(
            "smpl_tetraderon_indices_batch", smpl_tetraderon_indices_batch
        )
