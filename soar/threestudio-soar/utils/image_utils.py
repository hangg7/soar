...
from .graphics_utils import focal2fov, fov2focal
...
def resize_image(img, factor, mode="bilinear"):
    ...
    resize = torch.nn.functional.interpolate(
        resize[None].to(torch.float32), scale_factor=1 / factor, mode=mode
    )[0].to(dtype)
    if is_np:
        resize = resize.numpy()
    # print(type(img))
...
def world2scrn(xyz, cams, pad):
    utils_mod = load(
        name="cuda_utils", sources=["utils/ext.cpp", "utils/cuda_utils.cu"]
    )
    device = xyz.device
    ...
    view_mat = torch.cat([i.world_view_transform[None] for i in cams], 0).to(device)[
        :, None
    ]
    ...
    prj_mat = torch.cat([i.full_proj_transform[None] for i in cams], 0).to(device)[
        :, None
    ]
    ...
    reso = torch.cat(
        [torch.tensor([[[i.image_width, i.image_height]]]) for i in cams], 0
    ).to(device)
    ...
    if mask.mean() == 1:
        ...
        inMask = mask.permute([0, 2, 1])[mask_idx, scrnPos[..., 0], scrnPos[..., 1]].to(
            torch.bool
        )
...
def cross_sample(depth, cam_cur, mask_cur, cams, extractor, flatten=False):
    ...
    feat_spl = torch.nn.functional.grid_sample(
        feats, ndc[:, :, None], align_corners=False
    )
    # exit()
