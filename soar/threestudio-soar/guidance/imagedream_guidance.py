...
@threestudio.register("imagedream-multiview-diffusion-guidance")
    ...
    def append_extra_view(self, latent_input, t_expand, context, ip=None):
        ...
        if ip:
            ...
            context["ip_img"] = torch.cat(
                [ip_img, torch.zeros_like(ip_img)], dim=0
            )  # 2 * (batchsize + 1, c, h, w)
        ...
        **kwargs,
        ...
        if ref_rgb is not None:
            if comp_bg is not None:
                ip = ref_rgb * ref_mask + comp_bg * (1 - ref_mask)
            ip = T.functional.to_pil_image(ref_rgb)
            image_embeddings = self.model.get_learned_image_conditioning(ip)
            un_image_embeddings = torch.zeros_like(image_embeddings).to(
                image_embeddings
            )
        ...
        with torch.no_grad():
            ...
            if camera is not None:
                ...
                #     camera = torch.einsum("ij,njk->nik", ref_w2c, camera)
                # breakpoint()
            ...
            if ref_rgb is not None:
                context["ip"] = torch.cat(
                    [
                        ...
                        un_image_embeddings.repeat(batch_size, 1, 1),
                    ],
                    dim=0,
                ).to(text_embeddings)

            if extra_view:
                latent_model_input, t_expand, context = self.append_extra_view(
                    latent_model_input, t_expand, context, ip=ip
                )
        ...
        if extra_view:
            ...
            noise_pred_text, noise_pred_uncond = remove_extra_view(
                noise_pred_text
