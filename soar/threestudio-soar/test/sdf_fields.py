...
class HashMLPSDFField(Field):
    ...
    def reset_field(
        self, xyzs: Tensor, gt_shs: Tensor, gt_scales: Tensor, gt_quats: Tensor, weights: Tensor = None, ori_colors: Tensor = None
    ):
        ...
        # ori_colors = self(xyzs)['shs'].detach()
        ...
        for i in range(1000):
            ...
            # loss = (((out_shs - gt_shs_) ** 2) * (1 - weights) ).mean() + ((out_shs - ori_colors) ** 2 * (weights)).mean()
            loss = (
                ((out_shs - gt_shs) ** 2).mean()
