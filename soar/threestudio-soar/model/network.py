...
class POP_no_unet(nn.Module):
    def __init__(
        ...
        geom_proc_layers = {
            "unet": UnetNoCond5DS(
                c_geom, c_geom, nf, up_mode, use_dropout
            ),  # use a unet
            "conv": GeomConvLayers(
                c_geom, c_geom, c_geom, use_relu=False
            ),  # use 3 trainable conv layers
            "bottleneck": GeomConvBottleneckLayers(
                c_geom, c_geom, c_geom, use_relu=False
            ),  # use 3 trainable conv layers
        }
        ...
        self.decoder = ShapeDecoder(
            in_size=uv_feat_dim + c_geom, hsize=hsize, actv_fn="softplus"
        )
    ...
    def forward(self, pose_featmap, geom_featmap, uv_loc):
        ...
        N_subsample = (
            1  # inherit the SCALE code custom, but now only sample one point per pixel
        )
        ...
        pix_feature = (
            pix_feature.view(B, C, -1)
            .expand(N_subsample, -1, -1, -1)
            .permute([1, 2, 3, 0])
