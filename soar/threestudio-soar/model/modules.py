...
class CBatchNorm2d(nn.Module):
    ...
    def __init__(self, c_dim, f_channels, norm_method="batch_norm"):
        ...
        self.conv_gamma = nn.Conv1d(
            c_dim, f_channels, 1
        )  # match the cond dim to num of feature channels
    ...
    def forward(self, x, c):
        ...
        # Affine mapping
        gamma = self.conv_gamma(c).unsqueeze(
            -1
        )  # make gamma be of shape [batch, f_dim, 1, 1]
...
class GeomConvLayers(nn.Module):
    ...
    def __init__(self, input_nc=16, hidden_nc=16, output_nc=16, use_relu=False):
        ...
        self.conv1 = nn.Conv2d(
            input_nc, hidden_nc, kernel_size=5, stride=1, padding=2, bias=False
        )
        self.conv2 = nn.Conv2d(
            hidden_nc, hidden_nc, kernel_size=5, stride=1, padding=2, bias=False
        )
        self.conv3 = nn.Conv2d(
            hidden_nc, output_nc, kernel_size=5, stride=1, padding=2, bias=False
        )
        if use_relu:
            self.relu = nn.LeakyReLU(0.2, inplace=True)
...
class GeomConvBottleneckLayers(nn.Module):
    ...
    def __init__(self, input_nc=16, hidden_nc=16, output_nc=16, use_relu=False):
        ...
        self.conv1 = nn.Conv2d(
            input_nc, hidden_nc, kernel_size=4, stride=2, padding=1, bias=False
        )
        self.conv2 = nn.Conv2d(
            hidden_nc, hidden_nc * 2, kernel_size=4, stride=2, padding=1, bias=False
        )
        self.conv3 = nn.Conv2d(
            hidden_nc * 2, hidden_nc * 4, kernel_size=4, stride=2, padding=1, bias=False
        )

        self.up1 = nn.ConvTranspose2d(
            hidden_nc * 4, hidden_nc * 2, kernel_size=4, stride=2, padding=1, bias=False
        )
        self.up2 = nn.ConvTranspose2d(
            hidden_nc * 2, hidden_nc, kernel_size=4, stride=2, padding=1, bias=False
        )
        self.up3 = nn.ConvTranspose2d(
            hidden_nc, output_nc, kernel_size=4, stride=2, padding=1, bias=False
        )
...
class UnetNoCond5DS(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = Conv2DBlock(
            input_nc, nf, 4, 2, 1, use_bias=False, use_bn=False, use_relu=False
        )
        ...
        self.upconv1 = UpConv2DBlock(
            8 * nf, 8 * nf, 4, 2, 1, up_mode=up_mode
        )  # 2x2, 512
        self.upconv2 = UpConv2DBlock(
            8 * nf * 2, 4 * nf, 4, 2, 1, up_mode=up_mode, use_dropout=use_dropout
        )  # 4x4, 512
        self.upconv3 = UpConv2DBlock(
            4 * nf * 2, 2 * nf, 4, 2, 1, up_mode=up_mode, use_dropout=use_dropout
        )  # 8x8, 512

        # Coord regressor
        self.upconv4 = UpConv2DBlock(2 * nf * 2, 1 * nf, 4, 2, 1, up_mode=up_mode)  # 16
        self.upconv5 = UpConv2DBlock(
            1 * nf * 2, output_nc, 4, 2, 1, use_bn=False, use_bias=True, up_mode=up_mode
        )  # 32

        if return_2branches:
            self.upconvN4 = UpConv2DBlock(
                2 * nf * 2, 1 * nf, 4, 2, 1, up_mode=up_mode
            )  # 16
            self.upconvN5 = UpConv2DBlock(
                1 * nf * 2,
                output_nc,
                4,
                2,
...
class UnetNoCond6DS(nn.Module):
    def __init__(
        ...
        self.upconv2 = UpConv2DBlock(
            8 * nf * 2, 8 * nf, 4, 2, 1, up_mode=up_mode, use_dropout=use_dropout
        )  # 4x4, 512
        self.upconv3 = UpConv2DBlock(
            8 * nf * 2, 8 * nf, 4, 2, 1, up_mode=up_mode, use_dropout=use_dropout
        )  # 8x8, 512
        self.upconv4 = UpConv2DBlock(
            4 * nf * 3, 4 * nf, 4, 2, 1, up_mode=up_mode, use_dropout=use_dropout
        )  # 8x8, 512

        # Coord regressor
        self.upconvC5 = UpConv2DBlock(
            2 * nf * 3, 2 * nf, 4, 2, 1, up_mode="upsample"
        )  # 16
        self.upconvC6 = UpConv2DBlock(
            1 * nf * 3,
            output_nc,
            4,
            2,
        ...
        if return_2branches:
            # Normal regressor
            self.upconvN5 = UpConv2DBlock(
                2 * nf * 3, 2 * nf, 4, 2, 1, up_mode="upconv"
            )  # 32x32, 256
            self.upconvN6 = UpConv2DBlock(
                1 * nf * 3, 3, 4, 2, 1, use_bn=False, use_bias=True, up_mode="upconv"
            )  # 64x64, 128
...
class Unet(nn.Module):
    def __init__(
        ...
        # Coord regressor
        self.upconvC5 = UpConv2DBlock(
            4 * nf * 2, 2 * nf, 4, 2, 1, up_mode="upsample"
        )  # 16
        self.upconvC6 = UpConv2DBlock(
            2 * nf * 2, 1 * nf, 4, 2, 1, up_mode="upsample"
        )  # 32
        self.upconvC7 = UpConv2DBlock(
            1 * nf * 2,
            output_nc,
            4,
            2,
        ...
        if return_2branches:
            # Normal regressor
            self.upconvN5 = UpConv2DBlock(
                4 * nf * 2, 2 * nf, 4, 2, 1, up_mode="upconv"
            )  # 32x32, 256
            self.upconvN6 = UpConv2DBlock(
                2 * nf * 2, 1 * nf, 4, 2, 1, up_mode="upconv"
            )  # 64x64, 128
            self.upconvN7 = UpConv2DBlock(
                1 * nf * 2, 3, 4, 2, 1, use_bn=False, use_bias=True, up_mode="upconv"
            )  # 64x64, 128
...
def gen_transf_mtx_full_uv(verts, faces):
    ...
    uu = (
        v2 - v1
    )  # u axis of local coords is the first edge, [batch, uv_size, uv_size, 3]
    vv = v3 - v1  # v axis, second edge
    ww_raw = torch.cross(uu, vv, dim=-1)
    ww = F.normalize(ww_raw, p=2, dim=-1)  # unit triangle normal as w axis
    ww_norm = (
        torch.norm(uu, dim=-1).mean(-1).mean(-1)
        + torch.norm(vv, dim=-1).mean(-1).mean(-1)
    ) / 2.0
...
def gen_transf_mtx_from_vtransf(vtransf, bary_coords, faces, scaling=1.0):
    ...
    """
    #
    vtransf_by_tris = vtransf[
        :, faces
    ]  # shape will be [batch, uvsize, uvsize, 3, 3, 3], where the the last 2 dims being the transf (pure rotation) matrices, the other "3" are 3 points of each triangle
    transf_mtx_uv_pts = torch.einsum(
        "bpqijk,pqi->bpqjk", vtransf_by_tris, bary_coords
    )  # [batch, uvsize, uvsize, 3, 3], last 2 dims are the rotation matix
    ...
    def create_embedding_fn(self):
        ...
        for freq in freq_bands:
            for p_fn in periodic_fns:
                embed_fns.append(
                    lambda x, p_fn=p_fn, freq=freq: p_fn(math.pi * x * freq)
                )
                # embed_fns.append(lambda x, p_fn=p_fn, freq=freq:p_fn(x * freq))
                out_dim += self.input_dims
