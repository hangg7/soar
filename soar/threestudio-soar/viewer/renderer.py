...
from .utils import CameraState, InterruptRenderException, LightState, set_trace_context
        ...
        while self.running:
            ...
            if not self._render_event.wait(0.2):
                self.submit(
                    RenderTask(
                        "static",
                        self.server.get_camera_state(self.client),
                        self.server.light_state,
                    )
                )
            ...
            try:
                with self.lock, set_trace_context(self._may_interrupt_trace):
                    ...
                    img = self.server.render_fn(
                        task.camera_state, img_wh, task.light_state
                    )
