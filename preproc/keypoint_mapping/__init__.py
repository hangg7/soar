...
KEYPOINTS_FACTORY = {
    ...
    "dwpose": dwpose.DWPOSE_KEYPOINTS,
    ...
    "pymafx_49": openpose.OPENPOSE_25_KEYPOINTS + pymafx_smplx.PYMAFX_SMPLX_KEYPOINTS,
...
def convert_kps(
    ...
    if isinstance(keypoints, torch.Tensor):
        ...
        def new_array_func(shape, value, device_data, if_uint8):
            if if_uint8:
                ...
                dtype = None
            if value == 1:
                return torch.ones(size=shape, dtype=dtype, device=device_data.device)
            elif value == 0:
                return torch.zeros(size=shape, dtype=dtype, device=device_data.device)
            else:
                raise ValueError
    ...
    if mask is not None:
        ...
        mask = new_array_func(
            shape=(keypoints.shape[-2],), value=1, device_data=keypoints, if_uint8=True
        )
    ...
    if original_mask is not None:
        ...
        assert original_mask.shape[0] == len(
            src_names
        ), f"The length of mask should be {len(src_names)}"

    mask = new_array_func(
        shape=(len(dst_names),), value=0, device_data=keypoints, if_uint8=True
    )

    dst_idxs, src_idxs, _ = get_mapping(src, dst, approximate, keypoints_factory)
    out_keypoints[:, dst_idxs] = keypoints[:, src_idxs]
    # NOTE(Hang Gao @ 05/02): HACK.
    if dst.lower() == "openpose_137":
        out_keypoints[:, 8] = 0.5 * (out_keypoints[:, 9] + out_keypoints[:, 12])
        out_keypoints[:, [9, 12], :2] = (
            out_keypoints[:, [9, 12], :2]
            + 0.25 * (out_keypoints[:, [9, 12], :2] - out_keypoints[:, [12, 9], :2])
            + 0.5
            * (
                out_keypoints[:, [8], :2]
                - 0.5 * (out_keypoints[:, [9, 12], :2] + out_keypoints[:, [12, 9], :2])
            )
        )
    out_shape = extra_dims + (len(dst_names), keypoints.shape[-1])
    out_keypoints = out_keypoints.reshape(out_shape)
    mask[dst_idxs] = (
        to_type_uint8_func(original_mask[src_idxs])
        if original_mask is not None
        else 1.0
    )
...
def get_mapping(
    ...
    """
    if (
        src in __KEYPOINTS_MAPPING_CACHE__
        and dst in __KEYPOINTS_MAPPING_CACHE__[src]
        and __KEYPOINTS_MAPPING_CACHE__[src][dst][3] == approximate
    ):
        ...
        if unmapped_names:
            warn_message = (
                f"Approximate mapping {unmapped_names}" + f" to {approximate_names}"
            )
            print(warn_message)
...
def get_flip_pairs(
    convention: str = "smplx", keypoints_factory: dict = KEYPOINTS_FACTORY
) -> List[int]:
...
def get_keypoint_idxs_by_part(
    part: str, convention: str = "smplx", keypoints_factory: dict = KEYPOINTS_FACTORY
) -> List[int]:
